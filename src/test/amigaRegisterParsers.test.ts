...
suite('Amiga Register Parsers - Comprehensive Tests', () => {
  ...
  suite('Blitter Register Edge Cases', () => {
    test('should handle BLTCON0 maximum shift and all channels', () => {
      ...
      assert.strictEqual(channelsValue?.value, 15, 'All channels enabled');
    ...
    test('should handle BLTCON1 line mode with all flags', () => {
      ...
      assert.strictEqual(textureValue?.value, 15, 'Maximum texture pattern');
  ...
  suite('Display Position Registers', () => {
    test('should handle VPOSR with different chip IDs', () => {
      ...
      assert.ok(ecs_chipId?.description?.includes('1000'), 'Should show chip ID in hex');
      assert.ok(aga_chipId?.description?.includes('1800'), 'Should show chip ID in hex');
    });
