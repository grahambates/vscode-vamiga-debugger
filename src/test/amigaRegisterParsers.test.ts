...
describe("Amiga Register Parsers - Comprehensive Tests", () => {
  ...
  describe("INTENA/INTREQ Register Edge Cases", () => {
    it("should handle all interrupt bits set", () => {
      ...
      const receiveBufferFullBit = bits.find(
        (b) => b.name === "11: RECEIVE_BUFFER_FULL",
      );
    ...
    it("should differentiate INTREQ vs INTENA", () => {
      ...
      const intenaMasterEnable = intenaList.find(
        (b) => b.name === "14: MASTER_ENABLE",
      );
  ...
  describe("BPLCON Complex Scenarios", () => {
    it("should handle BPLCON0 maximum bitplanes", () => {
      ...
      assert.strictEqual(
        bitplanesValue?.value,
        7,
        "BITPLANES=7 means 7 bitplanes",
      );
    ...
    it("should handle BPLCON1 maximum scroll values", () => {
      ...
      assert.strictEqual(
        pf2hValue?.value,
        63,
        "PF2H maximum: (0xF | 0x30) = 63",
      );
      assert.strictEqual(
        pf1hValue?.value,
        63,
        "PF1H maximum: (0xF | 0x30) = 63",
      );
    });
  ...
  describe("Display Position Registers", () => {
    it("should handle VPOSR with different chip IDs", () => {
      ...
      assert.strictEqual(
        ecs_chipId?.value,
        "0x1000",
        "ECS chip ID from (0x2001 >> 1) & 0x7FFF",
      );
      assert.strictEqual(
        aga_chipId?.value,
        "0x1800",
        "AGA chip ID from (0x3001 >> 1) & 0x7FFF",
      );
    });
  ...
  describe("Register Detection and Routing", () => {
    it("should correctly identify all supported registers", () => {
      ...
      supportedRegs.forEach((regName) => {
        assert.ok(
          registerParsers.hasRegisterBitBreakdown(regName),
          `${regName} should be supported`,
        );
    ...
    it("should reject unsupported registers", () => {
      ...
      unsupportedRegs.forEach((regName) => {
        assert.ok(
          !registerParsers.hasRegisterBitBreakdown(regName),
          `${regName} should not be supported`,
        );
        ...
        assert.strictEqual(
          bits.length,
          0,
          `${regName} should return empty array`,
        );
      });
    });
    ...
    it("should have valid bit field structure", () => {
      ...
      intreqBits.forEach((bit) => {
        ...
        assert.ok(
          ...
          `${bit.name} name should be string`,
        );
      });
    });
  });
